...
print_telegram_icon() {
    echo -e "          ${MAGENTA}${ICON_TELEGRAM} lets talk on Telegram!${RESET}"
}
...
display_ascii_new() {
    echo -e "    ${RED} _____ ____  _   _ _   _ _  _____   _   _ ______     _____     __ ${RESET}"
    echo -e "    ${GREEN}| ____| __ )| | | | \ | | |/ /_ _| | | | |__  / |   / _ \ \   / / ${RESET}"
    echo -e "    ${BLUE}|  _| |  _ \| | | |  \| | ' / | |  | | | | / /| |  | | | \ \ / /  ${RESET}"
    echo -e "    ${YELLOW}| |___| |_) | |_| | |\  | . \ | |  | |_| |/ /_| |__| |_| |\ V /   ${RESET}"
    echo -e "    ${MAGENTA}|_____|____/ \___/|_| \_|_|\_\___|  \___//____|_____\___/  \_/    ${RESET}"
}
...
show_menu() {
    ...
    echo -e "    ${CYAN}8.${RESET} ${ICON_INSTALL} Install Node from list"
    echo -e "    ${CYAN}0.${RESET} ${ICON_EXIT} Exit"
    echo
    draw_bottom_border
    echo -ne "    ${YELLOW}Enter your choice [0-8]:${RESET} "  # Updated range to [0-7]
    read choice
}
...
install_node() {
    ...
    echo -ne "${YELLOW}Enter the number of networks:${RESET} "
    read networks

    # Run script.py with IP address and number of nodes
    python3 script.py "$ip_address" "$num_nodes" "$networks"
...
install_from_wallets_file() {
    echo -e "${GREEN}🛠️  Installing Node from list...${RESET}"
    # Update and upgrade the system
    sudo apt update && sudo apt upgrade -y

        # Check if file wallets.txt exists
    if [ ! -f "wallets.txt" ]; then
        echo -e "${RED}wallets.txt file not found.${RESET}"
        exit 1
    fi
    ...
    # Check number of lines in wallets.txt for set number of nodes variable
    num_nodes=$(wc -l < "wallets.txt")
